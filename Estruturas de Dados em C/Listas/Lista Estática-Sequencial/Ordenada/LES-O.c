...
Descrição: insere 'elem' de forma ordenada na lista, se possível
...
int insereOrd(Lista lst, int elem)
{
	...
	// lista está vazia ou o elemento é maior ou igual ao última (insere no final
	if(listaVazia(lst) == 1 || elem >= lst->vet[lst->fim - 1])
		lst->vet[lst->fim] = elem;
	else
	{
		int aux = 0, i;
	
		// percorre até achar o primeiro elemento maior que 'elem'
		while(elem >= lst->vet[aux])
			aux++;
			
		// desloca os elementos para a direita de modo a abrir espaço
		for(i = lst->fim; i > aux; i--)
			lst->vet[i] = lst->vet[i - 1];
			
		lst->vet[aux] = elem; // inclui o elemento na lista
	}
...
{
	if(lst == NULL || listaVazia(lst) == 1 || elem < lst->vet[0] || elem > lst->vet[lst->fim - 1])
		return 0; // lista vazia, elem menor que o primeiro, elem maior que o último

	int i, aux = 0;

	// percorre até encontrar um valor que seja igual ao elemento ou maior que ele
